...
| `_dispatch_ref` | `object` | **Periodic orchestrator — internal (v2).** The native delegated unit currently working this task. Shape: `{kind: "subagent" \| "workflow" \| "background", id: "<harness-unit-or-workflow-run-id>", started_at (ISO-8601)}`. Written by the orchestrator on top-up (§6d); cleared on reap. Its **presence** is what marks a task In-flight (the role the retired `_coordinator_team_name` played). Completion is detected from durable Taskforge state (terminal `task.status` + `attrs.completion`) plus native completion notifications — never a `/tmp/*.done` file or stdout marker. Underscore-prefixed. |
| `_coordinator_team_name` | `string` | **Legacy (v4–v6) — tolerated, never written by v2.** The old Teams teammate name for a `claude -p` coordinator. Superseded by `_dispatch_ref`. Tasks that still carry it are treated as Fresh (or Resumable if a usable `attrs.checkpoint` is present); the orchestrator ignores the key and clears it on reclaim. |
| `_coordinator_tmux_window` | `string` | **Legacy (tmux era) — tolerated, never written by v2.** The retired tmux window name / pane-scrape marker. Ignored on read; cleared on reclaim. |
| `_coordinator_task_id` | `string` | **Legacy (v3) — tolerated, never written by v2.** The old Claude Code background-Agent task id. Tasks with this attr set are treated as orphans on the next tick and the key is cleared on reclaim. |
| `checkpoint` | `object` | **Periodic orchestrator.** Set by the delegated unit at each phase boundary; read by the orchestrator during TOP-UP to decide resume vs fresh dispatch. Schema: `{workflow, workflow_version (UUID FK to workflow_versions.id), phases_completed (list), current_phase, updated_at (ISO-8601), by_coord (delegated-unit name, e.g. `coord-<short-id>`)}`. Cleared by the orchestrator on REAP after a successful `done` release. Per-workflow extensions (e.g., `migration_revision_id` for schema-migration) are allowed as additional keys. See `docs/designs/wfe-ckpt-2-helper.md` for the full checkpoint-helper + `--resume` contract. |
