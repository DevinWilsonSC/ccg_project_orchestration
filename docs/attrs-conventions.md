...
| `workflow_slug` | `string` | **TemplateTask spawned children.** The slug of the workflow that governs execution of this task, written by the orchestrator at intake when the task is a child spawned from a TemplateTask. Takes precedence over architect best-fit scoring: if set, the orchestrator uses this slug to resolve the workflow version rather than re-scoring. Falls back to best-fit if unset. See [Precedence rule](#workflow_slug-precedence-rule) below. |
...
### `workflow_slug` precedence rule

When resolving which workflow to run for a task:

1. **`attrs.workflow_slug` is set** → use the named workflow slug to look up the published version. Skip best-fit scoring entirely.
2. **`attrs.workflow_slug` is absent** → fall back to architect best-fit scoring against available published workflow versions.

If `attrs.workflow_slug` disagrees with `attrs.workflow` (the explicit override key), `workflow_slug` wins because it represents the structural intent of the template that spawned this task, while `workflow` is an ad-hoc operator override intended for the orchestrator's top-level policy, not task-level routing.

**Task types that use `workflow_slug`:** any task spawned from a TemplateTask (`template_id` is set on the subtree root). The orchestrator writes `workflow_slug` during the TOP-UP phase when it instantiates children from a template that declares a `workflow` key.

**Example:**

```json
{
  "template_id": "018f1234-...",
  "template_version": "018f5678-...",
  "workflow_slug": "six-phase-build",
  "_local_key": "backend-build",
  "repo_path": "/home/ubuntu/agile_tracker",
  "branch": "task/abc123-backend-build"
}
```

When the orchestrator picks up this task, it resolves `six-phase-build` → latest published version of that workflow, then spawns the coordinator prompt. If `six-phase-build` has been deleted or unpublished, the orchestrator falls back to best-fit and logs a warning.
