...
| `checkpoint` | `object` | **Periodic orchestrator.** Set by the delegated unit at each phase boundary; read by the orchestrator during TOP-UP to decide resume vs fresh dispatch. Schema: `{workflow, workflow_version (UUID FK to workflow_versions.id), phases_completed (list), current_phase, updated_at (ISO-8601), by_coord (delegated-unit name, e.g. `coord-<short-id>`)}`. Cleared by the orchestrator on REAP after a successful `done` release. Per-workflow extensions (e.g., `migration_revision_id` for schema-migration) are allowed as additional keys. See `docs/designs/wfe-ckpt-design.md` for the full contract. |
