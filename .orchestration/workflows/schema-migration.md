...
```bash
python3 scripts/checkpoint_phase.py "{{ task_id }}" "DESIGN (python-expert migration plan)"
```
...
**CHECKPOINT (run this first, before any other action in this phase):**
```bash
python3 scripts/checkpoint_phase.py "{{ task_id }}" "MIGRATION REVIEW (software-architect gate)"
```
If this command exits non-zero, stop: `add_note` the error output, then `release(blocked)`.
...
**CHECKPOINT (run this first, before any other action in this phase):**
```bash
python3 scripts/checkpoint_phase.py "{{ task_id }}" "INTEGRATE (alembic heads gate mandatory)"
```
If this command exits non-zero, stop: `add_note` the error output, then `release(blocked)`.
